...
# Logging setup
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='hedge.log', filemode='a')

BASE_URL = os.getenv("API_BASE_URL", "https://api-fxpractice.oanda.com/v3/")
CONTAINER_USERNAME = os.getenv("CONTAINER_USERNAME")  # each container will have its username
...
def validate_candles(token, expected_direction, instrument, webhook, model_name):
    ...
    if None in [pip_m1, pip_m5, pip_m15]:
        send_teams_alert("⚠️ Candle fetch error. Skipping validation.", webhook, model_name)
        return False

    msg = f"🔎 Candle Validation:\nM1: {pip_m1:.2f}, M5: {pip_m5:.2f}, M15: {pip_m15:.2f}"
    logging.info(f"{model_name}: {msg}")
    send_teams_alert(msg, webhook, model_name)
...
def place_hedge_order(original_side, original_units, instrument, hedge_id, hedge_token, multiplier, webhook, model_name):
    ...
    headers = {"Authorization": f"Bearer {hedge_token}", "Content-Type": "application/json"}
    body = {
        "order": {
    ...
    if r.status_code == 201:
        msg = f"🔄 Hedge placed: {hedge_side} {abs(hedge_units)} units"
        logging.info(f"{model_name}: {msg}")
        send_teams_alert(msg, webhook, model_name)
    else:
...
def send_teams_alert(message, webhook_url, model_name="HedgeBot"):
    logging.info(f"{model_name}: {message}")
    if not webhook_url:
        return
    payload = {"text": f"[{model_name}] {message}"}
    try:
        requests.post(webhook_url, headers={"Content-Type": "application/json"}, json=payload)
    except Exception as e:
        ...
        if not trade_hedge:
            if pips_loss <= loss_trigger:
                if validate_candles(token, "BUY" if side == "SELL" else "SELL", instrument, webhook, model_name):
                    place_hedge_order(side, units, instrument, hedge_id, hedge_token, multiplier, webhook, model_name)
                    send_teams_alert(f"🛡️ Hedge triggered at {pips_loss:.2f} pips loss.", webhook, model_name)
        else:
            pl_hedge = float(trade_hedge['unrealizedPL'])
            combined = unrealized + pl_hedge

            send_teams_alert(f"📈 Combined P/L: {combined:.2f}", webhook, model_name)
            if combined >= profit_trigger:
                close_all_trades(acc_id, token, instrument)
                close_all_trades(hedge_id, hedge_token, instrument)
                send_teams_alert(f"🏁 Profit target hit ({combined:.2f} pips). Trades closed.", webhook, model_name)
